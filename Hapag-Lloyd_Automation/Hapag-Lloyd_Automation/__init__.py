...
def makePDF(path):
    ...
    if 'GCT NEW YORK' in text[routingInfo+68:routingInfo+110]:
        ...
        discharge = 'MAHER'
    elif 'MAERSK-SEALAND TERMINAL' in text[routingInfo+68:routingInfo+110]:
        discharge = "APM"
#     elif '' in text[routingInfo+68:routingInfo+110]:
#         discharge = "ASI"
    else:
...
#         containerX.weight=containerTextX[x:y]
     ...
     #   descY = x
        ...
        elif containerX.description.find("COFFEE") != -1:
...
#                 containerX.overweightTier=4
            ...
            unknownSize = True
        contInfo = contRates[containerX.size]
        if w > contInfo.T4weight:
            containerX.overweightTier=4
        elif w > contInfo.T3weight:
            containerX.overweightTier=3
        elif w > contInfo.T2weight:
            containerX.overweightTier=2
        elif w > contInfo.T1weight:
            containerX.overweightTier=1
        else:
            containerX.overweightTier=0
...
#             subprocess.Popen("pdftk \"" + templateLocation + str(containerX.overweightTier) + containerX.overweight + containerX.reefer + ".pdf\" fill_form \"" + destinationOfFiles + str(containerX.containerCode) + "\" output \"" + destinationOfFiles + str(containerX.containerCode) + ".pdf\"").wait()
            subprocess.Popen("pdftk \"" + templateLocation + str(containerX.overweightTier) + ".pdf\" fill_form \"" + destinationOfFiles + str(containerX.containerCode) + "\" output \"" + destinationOfFiles + str(containerX.containerCode) + ".pdf\"").wait()
            fileLocations.append("\"" + destinationOfFiles + str(containerX.containerCode) + ".pdf\"")
        else:
            subprocess.Popen("pdftk \"" + templateLocation + str(containerX.overweightTier) + ".pdf\" fill_form \"" + destinationOfFiles + str(containerX.containerCode) + "\" output \"" + destinationOfFiles + str(containerX.containerCode) + "NON-STANDARD SIZE, PLEASE FIX" + ".pdf\"").wait()
...
def recursiveArrive(specificPath):
    for filename in os.listdir(specificPath):
        ...
        elif(os.path.isdir(specificPath+"\\"+filename)):
...
#     sys.argv = r"a J:\All motor routings\2018\Week 43\HAPAG".split()
    ...
    contRates = ContainerSizeInfo.loadValues(payout=True)[0]
#     values = ContainerSizeInfo.loadValues(True, True, True)
#     contRates = values[0]
#     laneCodes = values[1]
    
    
    if os.path.isdir(workOrderLocation):
        recursiveArrive(workOrderLocation)
    elif workOrderLocation[-4:] == ".pdf" or workOrderLocation[-4:] == ".PDF":
        makePDF(workOrderLocation)
...
# pyinstaller "C:\Users\ssleep\workspace\Hapag-Lloyd_Automation\Hapag-Lloyd_Automation\__init__.py" --distpath "J:\Spencer\Hapag-Lloyd_Automation" --noconsole -y